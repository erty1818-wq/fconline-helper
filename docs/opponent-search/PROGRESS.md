...
- 작업: OS-12 패드/키보드 분리 성적 + OS-13 포메이션별 성적 (둘 다 비교 탭, 같은 `Services/Splits.cs`로 묶음)
- 담당: Claude (2026-09-26)
- 시작한 것: `Splits.By(matches, ouid, key)` → 경기 수·승무패·승률·평균 득실. 키를 모르는 경기는 "알 수 없음"으로 남겨 합이 전체와 맞게 한다.
- 다음 단계: 코드 → 테스트 → 화면 확인 → 커밋(OS-12, OS-13 각각 표에 같은 커밋)
