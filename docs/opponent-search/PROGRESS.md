...
- 작업: OS-15 팀 단위 분석 (비교 탭)
- 담당: Claude (2026-09-26)
- 시작한 것: `Services/Teams.cs`. 규칙: 최신 경기부터 보며, 선발 11명 중 7명 이상이 어떤 팀의 기준 경기(그 팀의 가장 최근 경기) 선발과 같으면 그 팀, 아니면 새 팀. 팀별 성적(Splits 표)과 경기당 골+도움 상위 선수.
- 다음 단계: 코드 → 테스트(묶음 규칙) → 화면 확인 → 커밋
