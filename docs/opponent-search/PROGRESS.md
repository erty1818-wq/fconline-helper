...
- 작업: OS-14 포메이션별 대표 라인업 (비교 탭, 포메이션 표 아래)
- 담당: Claude (2026-09-26)
- 시작한 것: `Splits.Representative(matches, ouid, formation)` = 그 포메이션 경기 중 다른 경기들과 선발이 가장 많이 겹치는 실제 경기(메도이드). 2경기 이상 쓴 포메이션 최대 3개를 작은 피치로.
...
| OS-14 | 포메이션별 대표 라인업 | 진행 중 | | OS-13 다음 |
| OS-15 | 팀 단위 분석 | 대기 | | |
