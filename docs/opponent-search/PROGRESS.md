...
- 작업: OS-07 슛 종류별 성공률과 xG [추정]
- 담당: Claude (2026-09-26)
- 시작한 것: 로컬 캐시 공식경기 슛으로 로지스틱 모델(거리·골문 각도·헤더)을 맞추고 계수를 `Services/ExpectedGoals.cs`에 고정. 슈팅 탭에 종류별 시도·골·성공률 표와 xG 대비 실제 골.
- 다음 단계: 계수 맞추기(스크래치 파이썬) → 코드 → 테스트 → 화면 확인 → 커밋
