...
7. **가볍게 만든다.** 폴링과 타이머 루프를 쓰지 않는다. 필요한 순간에만 요청하고, 결과는 캐시한다.
   ...
   - 예외: 사용자가 요청한 **완전 자동**(`App/AutoWatcher.cs`)만 2초마다 본다. 그 모드일 때, 게임 창이 앞에 있을 때만, 점수판 부분만 읽는다. 다른 곳에 타이머를 늘리지 않는다.
8. **게임 규칙** (사용자 확인):
   - +11이 거래 최고 강화다.
   - 소속 팀컬러는 전원에게, 특성 팀컬러는 해당 카드에만 적용된다.
   - 급여 한도는 310이다.
   - 강화 팀컬러는 8명에서 최고 단계다.
   - 강화 팀컬러는 **카드마다 자기 강화 단계의 것 하나만** 받는다: 백금빛 물결 = 11강 이상, 금빛 = 8강 이상, 은빛 = 5강 이상, 동빛 = 3강 이상. +10 카드는 +11 카드가 많아도 금빛(+4)이다. 소속 팀컬러만 선발 전원에게 간다(`TeamColor.AppliesToSquad`).
   - 인게임 OVR = +1 기준 + 강화(+11은 +21) + 적응도(⑤는 +4) + 팀컬러(전체·세부) + 집중훈련(능력치 5~6개 +2, OVR 최대 +2). 폰 앱·데이터센터 대표팀 화면과 비교해 확인함(2026-09-26).
- **게임 규칙은 사용자에게 묻기 전에 공식 데이터센터에서 먼저 확인한다.** 예: 팀컬러 조건과 효과는 `https://fconline.nexon.com/datacenter/TeamColorDetail?teamcolorid=<id>`(헤더 `X-Requested-With: XMLHttpRequest`가 있어야 열림). 백금빛 10004, 금빛 10001, 은빛 10002, 동빛 10003.
...
- **릴리스:** `git tag vX.Y.Z && git push origin vX.Y.Z`를 하면 `.github/workflows/release.yml`이 exe를 빌드해 GitHub Releases에 올린다. 친구들 앱은 이걸 보고 업데이트한다.
  ...
  - 새 태그는 직전 태그보다 높아야 한다. 최신 태그는 `git fetch --tags && git tag --sort=-v:refname | head -1`로 확인한다(2026-09-26 기준 v0.10.1). **확인 결과를 본 다음에 번호를 정한다.** 확인과 태그 만들기를 한 명령에 묶지 않는다. 여러 AI와 사용자가 번갈아 배포하므로 기억한 번호는 틀릴 수 있다(2026-09-26에 v0.8.0이 이미 있는 줄 모르고 v0.7.2를 올린 적이 있다. 그래서 같은 커밋을 v0.8.1로 다시 올렸다). 새 기능이면 가운데 숫자, 고친 것만이면 끝 숫자를 올린다.
