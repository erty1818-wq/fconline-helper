...
public partial class PicksPage : UserControl
{
    ...
    private async void OnFind(object sender, RoutedEventArgs e)
    {
        ...
        await StudioKit.Run(FindButton, Status, async () =>
        {
            ...
            var picks = await squads.HiddenRankerPicksAsync(position, filter, StudioKit.IntOr(UsersBox, 10));
            ...
            Status.Text = picks.Count == 0 ? "조건에 맞는 카드가 없습니다. 최소 랭커 수를 낮추거나 조건을 넓혀 보세요." : $"{picks.Count}장 · 랭커 사용이 많고 싼 순서 · {Filters.Summary(filter)}";
        });
    }
