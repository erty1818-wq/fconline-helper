...
/// </summary>
public sealed record ShotDot(double Across, double Down, ShotResult Result, int Type, int SpId, int Minute, string MatchId, bool InBox);
    ...
    public static ShotMap Of(IEnumerable<MatchDetail> matches, string ouid, bool conceded)
    {
        ...
        foreach (var m in matches)
        {
            ...
            foreach (var s in side.ShootDetail)
            {
                ...
                dots.Add(new ShotDot(across, down, (ShotResult)s.Result, s.Type, s.SpId, GoalTime.ToMinute(s.GoalTime), m.MatchId, Pitch.IsInBox(s.X, s.Y)));
            }
        }
